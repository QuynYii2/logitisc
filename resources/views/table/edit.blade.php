...
@section('edit_table')
    <form method="POST" action="{{route('table.update')}}" >
        @csrf
            ...
            <div class="row">
                <div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="form-group">
                        Name Table
                        <input type="text" value="{{$detail_table->id}}" name="id" hidden>
                        <input type="text" id="quantity" name="list[0][]"  class="form-control" value="{{$detail_table->name}}">
                    </div>
                </div>
                <div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="form-group">
{{--                        @dd($detail_table->restaurant_id)--}}
                        Restaurant
                    @if($role_id->role_id == 1)
                            <select class="form-control" id="exampleFormControlSelect1" name="list[0][]">
                                <option value="{{$detail_table->restaurant_id}}">
                                    @foreach($list_restaurant as $values)
                                        {{$values->id == $detail_table->restaurant_id ? $values->name : ''}}
                                    @endforeach
                                 </option>
                                @foreach($list_restaurant as $values)
                        ...
                        @else
                            <select class="form-control" id="exampleFormControlSelect1" name="list[0][]" disabled>
                                @foreach($list_restaurant as $key => $values)
                                    <option value="{{$value->restaurant_id == $values->id ? $values->id : ''}}">
                                        {{$value->restaurant_id == $values->id ? $values->name : ''}}
                                    </option>
                                @endforeach
                            </select>
                        @endif
